...
load('trailList.mat')%load the batch file name
traillist = trailList;
for epoch_idx = 3:size(traillist,1)
    %%
    
    clearvars -except traillist epoch_idx data
    cd('D:\TuthillLab\video')%go to the data folder, it should contain chris_data.mat and the video you wish to analyze
    k = traillist(epoch_idx,:);
    nov = ([k,'_legCam_1.avi']); %name of the video wish to analyze
    trail_id = k; % trail ID in the data table
    ...
    cd('D:\TuthillLab\figure\220302\medail')%figure path
    mkdir(trail_id)
    cd(trail_id)
    close all
    V_conj = V';
